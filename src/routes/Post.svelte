...
import { setContext } from "svelte";
    ...
    const sortOptions: string[] = ["Nieuw", "Oud"];
    ...
    function toggleSort(): void {
        sortIndex = sortIndex === Sort.NEW ? Sort.OLD : Sort.NEW;
        sort(sortIndex);
        dropdownHidden = true;
    }
                        ...
                        <div>
                            ...
                            <ul class="options {dropdownHidden ? "hidden" : ""}">
                                {#each sortOptions as option}
                                    <li on:click={toggleSort} class="{sortOptions[sortIndex] === option ? "selected" :""}">{option}</li>
                                {/each}
                            </ul>
                        </div>
                    </div>
...
<style>
    ...
    .sort .options li:not(:last-of-type) {
